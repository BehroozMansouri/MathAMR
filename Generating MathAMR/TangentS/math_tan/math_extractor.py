...
class MathExtractor:
    ...
    def isolate_pmml(cls, tree):
        ...
        math_root = parsed_xml.find("math")  # namespaces have been remove
        if not math_root:
            math_root = parsed_xml.find("ltx_markedasmath")  # namespaces have been remove
    ...
    @classmethod
    def isolate_pmml_original(cls, tree):
        """
        extract the Presentation MathML from a MathML expr
    ...
    @classmethod
    def convert_mathml_slt_string(cls, mathml):
        pmml = cls.isolate_pmml(mathml)
        return SymbolTree(cls.convert_to_layoutsymbol(pmml)).tostring()
    ...
    @classmethod
    def parse_from_tex4(cls, tex, file_id=-1, position=[0]):
        """
        Parse expression from Tex string using latexmlmath to convert to presentation markup language
        ...
        return cls.isolate_cmml(mathml)

    @classmethod
    def parse_from_opt(cls, tex, file_id=-1, position=[0]):
        """
        Parse expression from Tex string using latexmlmath to convert to presentation markup language
